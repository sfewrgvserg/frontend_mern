...
const Popular = async ({
  ...
  if (limitProducts && categoryProducts) {
    data = await axios.get(
      `http://localhost:3000/api/products?limit=${limitProducts}&category=${categoryProducts}`
    );
  } else if (categoryProducts) {
    data = await axios.get(
      `http://localhost:3000/api/products?category=${categoryProducts}`
    );
  } else if (limitProducts) {
    data = await axios.get(
      `http://localhost:3000/api/products?limit=${limitProducts}`
    );
  } else {
    data = await axios.get(`http://localhost:3000/api/products`);
  }
